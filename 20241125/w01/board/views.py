...
def bwrite(request):
  if request.method == 'GET':
    ...
    id = request.session.get('session_id') # session_id의 값을 가져온다.
    member = Member.objects.get(id=id)
    btitle = request.POST.get('btitle')
    bcontent = request.POST.get('bcontent')
    bfile = request.FILES.get('bfile', '')
    print('파일이름1 : ', request.FILES)
    print('파일이름2 : ', bfile)
...
def breply(request, bno):
  if request.method == 'GET':
    ...
    # 같은 bgroup속한 데이터에서 bstep더 큰것만 검색하여 1씩 증가
    # bstep__gt 보다 큰거
    ...
    qs = Board(member = member, btitle = btitle, bcontent = bcontent,\
               bgroup = bgroup, bstep = int(bstep) + 1, bindent = int(bindent) + 1)
    qs.save()
    context = {'rmsg': '1'}
...
def bview(request, bno):
  ...
  # cookie_name 존재하면
  # 조회사구 증가하면 cookie_name 증가한 번호를 추가
  print('cookie_name : ', request.COOKIES.get('cookie_name'))
  if request.COOKIES.get('cookie_name') is not None:
    # 쿠키를 읽어와서 안에 1|3|4 -> 2번 1증가, 3번 증가 안됨
