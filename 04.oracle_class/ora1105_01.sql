...
-- employees 사원번호, 사원명, 부서번호, 부서명을 출력하세요.
...
-- avg값을 가지고 다른 컬럼을 다른 테이블에서 가져와 출력하세요.
...
-- avg의 컬럼을 가지고, stu_grade 활용하여 result에 값을 입력하세요.
-- non-equi join update 구문
update stu set result = (
select gr from(select no, grade gr from stu, stu_grade
...
update stu set rank = (
select ranks from (select no, rank() over(order by avg desc) ranks from stu) b
where stu.no = b.no
);
...
-- self join manager_id, 매니저의 이름을 출력하세요.
-- outer join : 해당 컬럼에 null 값이 존재하면 null값 까지 포함하여 출력
...
select employee_id, emp_name, a.manager_id, emp_name
from employees a left outer join departments b
on a.department_id = b.department_id;

-- 오라클 outer-join : 두개의 컬럼의 (+)는 에러 둘중 하나만 해야함
select employee_id, emp_name, a.department_id, department_name
from employees a, departments b
where a.department_id(+) = b.department_id;
...
-- employees 에 없는 salary 5000 and 8000 작은 사원의 부서번호, 부서이름
-- employees에 없는 departments의 부서 검색 -> 부서번호, 부서이름
select department_id, department_name
from departments a
where not exists
...
select * from board2;
-- 8,11,12,16,21,22,25,29,35,38,44,46,57,61,66,74,88,95,96,98 번 
...
-- rownum: 번호를 새롭게 부여
select rownum, bno, btitle from board2
order by btitle;

select rownum, bno, btitle from(
select bno, btitle from board2 order by btitle
);
...
select * from(
select row_number() over(order by bdate desc) runm, a.* from board2 a
) where runm between 11 and 20;
...
select rownum rnum, a.* from (
select rownum rnum, a.* from board2 a order by bdate desc) a
;
...
order by avg desc)a
where students.no = a.no
);

commit;

-- view
-- 상담원 : 사원들 전화를 가지고 사원들에게 마케팅을 하려고 한다.
...
-- 사원번호, 사원명, 이메일, 폰번호, 입사일, 부서번호, 부서명 출력하세요.
select a.*, b.department_id, b.department_name from employees_view a, departments b;
...
-- insert, update, delete 가능, not null 제약조건이 되어 있으면 insert불가능할수도 있음
-- 복합 view : 2개 이상 테이블조인, 함수사용, group by 같은 경우는 insert, update, delete 불가능
-- 100번 이름 홍길동으로 번경하세요.
...
select no, name, kor, eng, math, total, round(avg, 2), rank, substr(sdate,0,2)
from students;
